...
def run_coherence_a(q, label, noise1, noise2, nfreqs):
    ...
    q.put(cpu_time)  # Send CPU time in seconds
...
if __name__ == "__main__":
    ...
    peaks_a = []
    peaks_b = []
    #nfreqs = ([50,100,150,200,250,300,350,400,450,500])
    nfreqs = 100
    ...
    for d in durations:
        ...
        noise2 =np.random.randn(len(t))
        #n = int(n)
        pa = Process(target=run_coherence_a, args=(qa, 'Test A', noise1, noise2, nfreqs))
        pb = Process(target=run_coherence_b, args=(qb, 'Test B', noise1, noise2, nfreqs))
